...
class PostEntitySpec extends ScalaTestWithActorTestKit(PostEntitySpec.config)
  ...
  private val aPost = Post(1, "Blablabla", "data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQABAAD...", DateTime.now.toString(), "test_testson@testmail.com")
  ...
  "The post entity" should {
    ...
    "save a new post" in {
      val result = eventSourcedTestKit.runCommand[StatusReply[Post]](replyTo => PostEntity.PostPost(aPost, replyTo))
      result.reply should ===(StatusReply.Success(aPost))
      result.event should ===(PostEntity.PostPosted(aPost))
    }
  }
}
